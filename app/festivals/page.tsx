...
export default async function FestivalsPage({ searchParams }: { searchParams: Promise<{ region?: string; query?: string }> }) {
  ...
  const eventFestivals: Festival[] = getAllEvents()
    .filter((event) => event.genreKey === "festival" && event.endDate >= new Date().toISOString().slice(0, 10).replaceAll("-", ""))
    .map((event) => ({
      id: `event-${event.id}`,
      title: event.title,
      addr: event.address || event.place || "",
      area: event.area,
      image: event.imgUrl || "",
      mapx: event.gpsX || "",
      mapy: event.gpsY || "",
      ...
      description: event.contents || `${event.area}에서 열리는 축제·행사입니다.`,
      place: event.place,
      homepage: event.officialUrl,
      tel: event.phone,
      source: "공공 문화행사 데이터",
    }));
  const festivals = [...collected, ...eventFestivals].filter((festival, index, all) => all.findIndex((item) => item.title === festival.title && item.startDate === festival.startDate) === index);
