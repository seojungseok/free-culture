...
export default function WeekendPage(){
 ...
 const allEvents:WeekendCandidate[]=getWeekend().map(e=>({id:'event:'+e.id,title:e.title,href:'/event/'+e.id,area:e.area,kind:'event',image:e.imgUrl,address:e.address||e.place,start:e.startDate,end:e.endDate,free:e.priceType==='free',kids:e.audiences?.includes('kids')||false,nature:false,seasonal:false}));
 ...
 const regions=['서울','경기','인천','부산','제주'].map(area=>({area,code:(SIDO_SLUG as Record<string,string>)[area],events:allEvents.filter(e=>e.area===area)})).filter(region=>region.code&&region.events.length);
 ...
 <section className="mt-10"><h2 className="text-xl font-bold">지역별 이번 주말</h2><p className="mt-2 text-sm text-ink-soft">행사와 나들이 장소, 코스를 지역별로 함께 살펴보세요.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{regions.map(region=><Link key={region.area} href={`/region/${region.code}`} className="rounded-xl border p-4 hover:border-brandblue"><strong>{region.area} 이번 주말 가볼 만한 곳</strong><span className="mt-1 block text-xs text-ink-soft">기간에 맞는 행사 {region.events.length}건과 지역 나들이 보기 →</span></Link>)}</div></section>
