...
export default function PetTravelPage() {
  ...
  const initial = getPetTravelPlaces().map(p => ({id:p.id,title:p.title,address:p.address || p.addr || '',area:p.area || '',image:p.image || '',type:p.type || '',summary:petOverview(p).slice(0,180)}));
