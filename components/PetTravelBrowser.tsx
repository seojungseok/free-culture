...
export default function PetTravelBrowser() {
  ...
  const regionCounts = useMemo(() => items.reduce<Record<string, number>>((counts, item) => { if (item.area) counts[item.area] = (counts[item.area] || 0) + 1; return counts; }, {}), [items]);
  return (
    <div className="bg-panel"><div className="mx-auto max-w-[1180px] px-5 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">{regions.map((r) => <button key={r} onClick={() => setRegion(r)} className={["min-h-9 shrink-0 rounded-full px-3.5 text-[13px] font-bold", region === r ? "bg-free text-white" : "border border-line bg-white text-ink-soft"].join(" ")}>{r} <span className="ml-0.5 text-[11px] opacity-75">{r === "전체" ? items.length : regionCounts[r] || 0}</span></button>)}</div>
