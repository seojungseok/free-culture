...
export default function PrepImage({photo,products,priority=false,onPlace}:{photo:Photo;products:PrepProduct[];priority?:boolean;onPlace?:(x:number,y:number)=>void}){
 ...
 return <figure className="prep-photo" onKeyDown={e=>{if(e.key==='Escape')close();}}>
  ...
  <figcaption>{photo.generated?(photo.productMatchReviewed?'실제 상품 사진을 참고한 AI 연출 이미지 · 정확한 규격과 구성은 상품 정보를 확인하세요.':'AI 연출 이미지 · 실제 판매 상품과 외형·구성이 다를 수 있습니다.'):photo.alt}{photo.tags.length>0&&(onPlace?' 사진의 +를 눌러 연결을 검토하세요.':' 사진의 +를 누르면 해당 쿠팡 제휴 상품페이지가 새 창으로 열립니다.')}</figcaption>
