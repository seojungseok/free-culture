...
export default function WeekendDiscovery({items,start,end,seasonLabel}:{items:WeekendCandidate[];start:string;end:string;seasonLabel:string}){
 ...
 {results.length>limit&&<button onClick={()=>setLimit(n=>n+6)} className="mx-auto mt-6 block min-h-11 rounded-xl border px-6 font-bold">후보 6곳 더 보기</button>}
 <nav className="mt-5 flex flex-wrap gap-4 text-sm font-bold text-brandblue" aria-label={`${area} 전체 정보 더 보기`}><Link href={`/events${(SIDO_SLUG as Record<string,string>)[area]?`?region=${(SIDO_SLUG as Record<string,string>)[area]}`:''}`}>{area} 문화행사 전체 보기 →</Link>{(SIDO_SLUG as Record<string,string>)[area]&&<Link href={`/region/${(SIDO_SLUG as Record<string,string>)[area]}`}>{area} 나들이·코스 보기 →</Link>}</nav></>}</section>
