...
export default function NextStop({anchor:input,camping=false}: {anchor:TripStop;camping?:boolean}) {
  ...
  const regionSlug=(SIDO_SLUG as Record<string,string>)[anchor.area];
  ...
  return <section className="my-7 rounded-2xl bg-slate-50 p-4 sm:p-6" aria-label="일정 만들기">
    ...
    <div className="mt-4 flex flex-wrap gap-5 text-sm font-bold text-brandblue">{regionSlug&&<Link href={'/region/'+regionSlug} prefetch={false} className="inline-flex min-h-11 items-center">{anchor.area} 행사·나들이 함께 보기 →</Link>}<Link href="/plan" className="inline-flex min-h-11 items-center">날짜·조건으로 추천 받기 →</Link><Link href="/saved" className="inline-flex min-h-11 items-center">보관함 보기 →</Link></div>
  </section>;
}
