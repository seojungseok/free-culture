...
export default function QuickEntry({ seasonLabel, seasonEmoji, seasonTerms, dateThemes }: { seasonLabel: string; seasonEmoji: string; seasonTerms: string[]; dateThemes: DateTheme[] }) {
  ...
  const entries: { key: Key; emoji: string; label: string }[] = [
    { key: "loc", emoji: "📍", label: "내 주변" },
    { key: "free", emoji: "🆓", label: "무료 행사" },
    { key: "season", emoji: seasonEmoji, label: `${seasonLabel} 나들이` },
    { key: "date", emoji: "💑", label: "데이트 코스" },
  ];
  ...
  return (
    <div>
      ...
      {open === "season" && (
        <ThemePanel title={`${seasonEmoji} ${seasonLabel} 나들이`} hint="테마를 고르고(여러 개 가능) 지역을 누르세요 · 계절은 자동으로 바뀌어요"
