...
import { fmtMd, festivalHref } from "@/lib/festivals";
      ...
      {filtered.length ? (
        <div className="mt-3 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
          {filtered.map((f) => <article key={f.id} className="overflow-hidden rounded-xl border border-line bg-white shadow-sm">
            <a href={festivalHref(f)} className="block"><div className="aspect-[4/3] bg-tint">{f.image ? <img src={f.image} alt={`${f.title} 축제 현장 사진`} loading="lazy" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-4xl" aria-hidden="true">🎉</div>}</div></a>
            <div className="p-3 sm:p-4">
              ...
              <h2 className="mt-2 line-clamp-2 text-[14px] font-black leading-5 text-ink sm:text-[17px] sm:leading-6"><a href={festivalHref(f)} className="hover:text-free">{f.title}</a></h2>
              <p className="mt-2 text-[12px] font-bold text-ink-soft sm:text-[13px]">{formatRange(f.startDate, f.endDate)}</p>
              {(f.place || f.addr) && <p className="mt-1 line-clamp-1 text-[12px] leading-5 text-ink-faint sm:text-[13px]">{f.place || f.addr}</p>}
              {introFor(f) && <section className="mt-3 hidden border-t border-line pt-3 sm:block">
              ...
              <dl className="mt-3 hidden grid-cols-[58px_1fr] gap-x-2 gap-y-1 text-[12px] leading-5 sm:grid">
                ...
                {f.place && <><dt className="font-bold text-ink-faint">개최 장소</dt><dd className="text-ink-soft">{f.place}</dd></>}
                {f.addr && <><dt className="font-bold text-ink-faint">주소</dt><dd className="text-ink-soft">{f.addr}</dd></>}
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-[12px] font-bold sm:mt-4">
                <a href={festivalHref(f)} className="text-free">축제 정보 보기 →</a>
