...
if(!['thumbnail','body-1','body-2'].includes(slot))throw Error('허용 이미지 슬롯 아님');
...
try{
 const response=await fetch('https://api.openai.com/v1/images/generations',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:record.model,quality:record.quality,size:record.size,n:1,output_format:'jpeg',output_compression:90,prompt:`Create a travel editorial background. ${recipe.prompt}\nNo readable text, logos, price charts, timetable or map. No identifiable faces. Do not invent a real facility or present invented scenery as this venue. Without reliable exact visual evidence, use an editorial illustration clearly distinct from an actual photograph. Leave central safe space for a concise Korean title to be overlaid. Source facts: ${recipe.basis}`}),signal:AbortSignal.timeout(240000)});
 ...
 const composed=await sharp(base).composite([{input:svg}]).png().toBuffer();let bytes;for(const quality of [90,86,82,78]){bytes=await sharp(composed).jpeg({quality,mozjpeg:true}).toBuffer();if(bytes.length<=250*1024)break;}
