...
for(const day of new Set(state.history.map(h=>h.day)))if(state.history.filter(h=>h.day===day).length>(policy.historicalPublicationAllowances?.[day]?.limit||policy.dailyAdditionalLimit||20))throw Error('발행 한도 위반');
