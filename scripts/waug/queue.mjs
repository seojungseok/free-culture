...
if(command==='plan'){
 ...
 while(todo.length&&guard++<365){const fixed=jobs.filter(j=>j.publicationDay===day).length;const actual=day===today?publicationBudget().waugPublished:state.history.filter(h=>h.day===day).length;let slots=Math.max(0,20-fixed-actual);const areas={},themes={};for(const j of jobs.filter(j=>j.publicationDay===day)){areas[j.area]=(areas[j.area]||0)+1;themes[j.theme]=(themes[j.theme]||0)+1;}
