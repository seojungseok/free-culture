...
if(command==='plan'){
 ...
 day=nextDay(day);}
 const queue={version:1,timezone:'Asia/Seoul',dailyPublicationLimit:20,dailyNewWritingLimit:20,updatedAt:new Date().toISOString(),jobs};write('queue',queue);
 ...
 const started=previous.jobs.filter(j=>j.startedDay===today).length;const j=previous.jobs.find(j=>j.placeId===process.argv[3]);if(!j||!j.publicationDay||j.publicationDay>today||j.attempts>=3||['held','excluded','published'].includes(j.status))throw Error('실행 가능한 예약이 아님');
 if(!j.articleSlug&&j.startedDay!==today&&started>=20)throw Error('일일 신규 작성 상한');
