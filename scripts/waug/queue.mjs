...
const policy=read('publication-policy');
const dailyLimit=Number(policy.dailyAdditionalLimit)||20;
...
if(command==='plan'){
 ...
 while(todo.length&&guard++<365){const fixed=jobs.filter(j=>j.publicationDay===day).length;const actual=day===today?publicationBudget().waugPublished:state.history.filter(h=>h.day===day).length;let slots=Math.max(0,dailyLimit-fixed-actual);const areas={},themes={};for(const j of jobs.filter(j=>j.publicationDay===day)){areas[j.area]=(areas[j.area]||0)+1;themes[j.theme]=(themes[j.theme]||0)+1;}
 ...
 j.publicationDay=day;j.scheduledAt=`${day}T05:00:00+09:00`;j.writingAt=`${day}T00:17:00+09:00`;areas[j.area]=(areas[j.area]||0)+1;themes[j.theme]=(themes[j.theme]||0)+1;slots--;}
 day=nextDay(day);}
 const queue={version:1,timezone:'Asia/Seoul',dailyPublicationLimit:dailyLimit,dailyNewWritingLimit:dailyLimit,updatedAt:new Date().toISOString(),jobs};write('queue',queue);
 ...
 if(!hasDraft&&j.writingDay!==today){if(started>=dailyLimit)throw Error('일일 신규 작성 상한');j.writingDay=today;}
