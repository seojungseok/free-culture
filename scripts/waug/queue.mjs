...
if(command==='plan'){
 ...
 const started=previous.jobs.filter(j=>j.writingDay===today).length+publicationBudget().otherPublished;const j=previous.jobs.find(j=>j.placeId===process.argv[3]);if(!j||!j.publicationDay||j.publicationDay>today||j.attempts>=3||['held','excluded','published'].includes(j.status))throw Error('실행 가능한 예약이 아님');
 const hasDraft=state.articles.some(a=>a.placeId===j.placeId);
 if(!hasDraft&&j.writingDay!==today){if(started>=20)throw Error('일일 신규 작성 상한');j.writingDay=today;}
