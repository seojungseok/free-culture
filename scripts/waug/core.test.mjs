...
test('20 tickets per day; final day remainder; duplicate run is a no-op',()=>{
  ...
  assert.ok(state.articles.filter(a=>a.scheduledAt.startsWith('2026-09-11')).every(a=>a.scheduledAt.endsWith('T05:00:00+09:00')));
  assert.equal(publicArticles(state,products).length,0);
  const day1=new Date('2026-09-10T20:00:00Z');assert.equal(publish(state,products,day1).length,20);assert.equal(publish(state,products,day1).length,0);
  assert.equal(publish(state,products,new Date('2026-09-11T20:00:00Z')).length,15);
});
...
test('launch still respects the current daily ticket cap',()=>{
  ...
  const ids=state.articles.slice(0,20).map(a=>a.placeId);
  ...
  assert.equal(publish(state,products,new Date('2026-09-10T20:00:00Z')).length,20);
  assert.throws(()=>launchNow(state,products,ids,new Date('2026-09-10T20:00:00Z')));
});
