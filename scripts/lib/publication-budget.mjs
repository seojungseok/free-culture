...
export function publicationBudget(root=process.cwd(),now=new Date()){
 const day=dayKST(now), seen=new Set();
 for(const file of ['place-articles','course-articles','city-tour-articles']){
  const rows=read(root,`data/${file}.json`,{}).articles||{};
  for(const [id,a] of Object.entries(rows))if(a.publishedAt&&dayKST(new Date(a.publishedAt))===day&&a.status!=='draft'&&a.status!=='held')seen.add(file+':'+(a.id||id));
 }
 ...
 return {day,published:seen.size,reserved:reserved.size,remaining:Math.max(0,20-seen.size),unreserved:Math.max(0,20-seen.size-reserved.size),otherPublished:seen.size-editorial.history.filter(h=>h.day===day).length};
}
// Called before each new generation. A process may keep its store in memory until exit.
export function newArticleAllowance(root=process.cwd()){return publicationBudget(root).unreserved;}
