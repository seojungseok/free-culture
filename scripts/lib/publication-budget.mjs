...
export function publicationBudget(root=process.cwd(),now=new Date()){
 ...
 return {day,published:seen.size,reserved:reserved.size,remaining:Math.max(0,ticketPolicy.dailyAdditionalLimit-editorial.history.filter(h=>h.day===day).length),unreserved:Math.max(0,policy.general.daily-(seen.size-editorial.history.filter(h=>h.day===day).length)),categories,waugPublished:editorial.history.filter(h=>h.day===day).length,otherPublished:seen.size-editorial.history.filter(h=>h.day===day).length};
}
