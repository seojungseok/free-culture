...
export function qualityCheck(text, { overview = "", existingTexts = [], minimalMode = false } = {}) {
  ...
  // 상한 1900 — 목표 1,100~1,500자(정보성 블로그 분량)에 여유를 둔 값.
  if (len > 1900) return { ok: false, reason: `길이 ${len}자(너무 김)`, len };
...
- ★ 분량은 1,100~1,500자를 지향한다. 원본 근거가 충분하면 문장을 줄이지 말고, 오히려 근거 안에서 풀어 써 채운다. 근거가 빈약하면 억지로 늘리지 말고 짧게 둔다.
...
[분량 — 거짓 없이 "풀어서" 채우기]
- 원본이 풍부하면 1,100~1,500자(공백 제외). 원본이 빈약하면 400~600자도 좋습니다.
...
- 소제목 순서(근거가 있는 것만 쓰되 최소 4개):
  ## 어떤 곳인가요 → ## 볼거리·즐길거리 → (## 아이·가족과 함께라면) → (## 가는 길·주차) → ## 방문 팁
