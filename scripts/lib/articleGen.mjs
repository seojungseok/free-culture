...
"${course.area}"의 해수욕장을 순서대로 소개하는 **베스트 리스트 글**을 씁니다(동선 코스가 아니라 순위 소개).

[지역] ${course.area}
[대상] ${course.area} 해수욕장 ${n}곳(아래 순서대로)
[해수욕장 목록 — 각 '자료'에 있는 사실만 사용]
${items}

[✍️ 글 톤]
- 정보 전달형 안내체("~입니다/~예요/~로 유명합니다"). 1인칭 경험담("가봤어요/좋았어요") 금지.
- 각 해수욕장이 **어떤 곳인지·무엇으로 유명한지·어떤 사람에게 좋은지**를 자료 기반으로 소개.

[구조]
- 1번째 줄: "# ${course.area} 해수욕장 베스트 ${n} — {대표 2~3곳}" (검색어: "${course.area} 해수욕장", "${course.area} 바다 여행")
- 2번째 줄: 이 지역 바다의 매력을 요약한 한 문장 **굵게**.
- 도입 2~3문장: ${course.area} 해수욕장 여행의 매력·시기.
- 해수욕장마다 "## 1. ○○해수욕장" 소제목 + 4~6문장으로 **풍부하게**: 모래·수심·바다 분위기, 물놀이·일출/일몰·산책 등 즐길 거리, 어떤 사람(가족·연인·서핑 등)에게 좋은지, 주변에 함께 가볼 곳.
  - 각 항목 끝에 "주변 맛집·먹거리는 이 글 아래 '근처 맛집'에서 확인하세요"처럼 연결하고, ${course.area} 대표 먹거리를 일반 상식 선에서 곁들여도 좋습니다(특정 식당명 지어내기 금지).
- 마지막 "## 여행 팁": 성수기·주차·안전(이안류 등)·물놀이 준비물 등 실용 정보.

[읽기 편하게] 문단 2~3문장·빈 줄, 핵심 **굵게**, 어미 다양화. 지루하지 않게.
[사실] 해수욕장의 **일반적으로 널리 알려진 특징**(고운 모래, 얕은 수심, 백사장, 일출/일몰 명소, 서핑 스팟 등)은 자유롭게 서술해도 됩니다. 다만 **구체적 수치(백사장 길이·개장 연도)·특정 인물·상호는 자료에 없으면 쓰지 마세요.** 해수욕장 이름은 자료 그대로.
[금지] "잊지 못할 추억을 선사" 류 클리셰, 이모티콘, 광고 문구.
