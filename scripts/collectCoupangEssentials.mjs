...
//   ③ 물티슈·화장지 — 베스트 목록(약 95개)까지 안 들어와서 이 둘만 검색으로 보충(로테이션).
...
const FILL_KEYWORDS = ["물티슈 대용량", "캡형 물티슈", "3겹 화장지 30롤", "대용량 화장지"];

// 캠핑에서 실제로 쓰는 생필품만 남긴다(종류 판정 겸용). 위에서부터 먼저 맞는 것으로 분류.
const KINDS = [
  { kind: "물", label: "생수", re: /생수|샘물|먹는샘물|미네랄워터/ },
  { kind: "물티슈", label: "물티슈", re: /물티슈/ },
  { kind: "화장지", label: "화장지", re: /화장지|화장실용|롤휴지|두루마리|미용티슈|각티슈/ },
  { kind: "주방", label: "주방용품", re: /키친타월|키친타올|위생장갑|니트릴장갑|고무장갑|지퍼백|롤백|크린백|위생봉투|랩|호일|종이컵|종이용기|일회용\s?(숟가락|젓가락|수저|접시|용기)|위생행주|수세미/ },
  { kind: "정리", label: "생활용품", re: /쓰레기봉투|종량제|비닐봉투|세제|섬유유연제|핸드워시|물비누|손소독/ },
];
