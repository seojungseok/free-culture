...
for(const item of queue){
 ...
 try{
  const context={today,course:item.raw,places:item.related.map(p=>({...p,overview:String(overviews[p.id]||'').replace(/<[^>]*>/g,'').slice(0,1200)})),restaurants:item.foodLinks,restaurantSelection:'기존 한국관광공사 음식점 캐시에서 같은 지역의 경유지와 좌표 기준 약 2km 이내 후보를 선정했다. 도로 이동거리·영업여부·공식 투어 경유 여부는 검증하지 않았다.'};
