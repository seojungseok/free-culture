...
local n=tonumber(ARGV[2]); if n<1 or n>math.min(9,s.cap) then return {-2,0} end
if count+n>math.min(9,s.cap) then return {0,events[1].at+s.window-now+1} end
...
export async function limitedRequest(run,{count=1,command=redis,key=stateKey(),sleep=ms=>new Promise(r=>setTimeout(r,ms)),maxWait=Infinity}={}){
 if(!Number.isInteger(count)||count<1||count>9)throw Error('조회량은 1~9개만 허용 (직전 60초 10개 미만)');
