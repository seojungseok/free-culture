...
const plan=JSON.parse(fs.readFileSync('data/weekend-prep-stir-fry-plan.json','utf8'));
const output='data/weekend-prep-stir-fry-research.json';
...
for(const topic of plan.topics){
 for(const keyword of topic.keywords){
  ...
  report.records.push({slug:topic.slug,topic:topic.title,keyword,queriedAt:new Date().toISOString(),fromCache:response.headers.get('X-Prep-Cache')==='hit',status:!candidate?'no-result':!affiliateUrl(candidate.affiliateUrl)?'invalid-affiliate-link':'needs-photo-option-review',candidate});
  save();console.log(`${topic.slug} / ${keyword}: ${candidate?.name||'결과 없음'}`);
 }
}
console.log(`이번 실행 후보 ${attempts}개, 누적 ${report.records.length}개. 공용 제한 기록을 사용했습니다.`);
