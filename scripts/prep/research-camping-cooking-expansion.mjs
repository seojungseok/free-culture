...
const output = 'data/weekend-prep-camping-cooking-expansion-research.json';
...
for (const [key, keyword] of queries) {
  ...
  const candidate = found ? {
    id: String(found.productId), name: found.productName, image: found.productImage,
    affiliateUrl: found.productUrl, source: `https://www.coupang.com/vp/products/${found.productId}`,
    checkedAt: new Date().toISOString(), verified: false,
  } : null;
  ...
  console.log(`${key}: ${candidate?.name || '결과 없음'}`);
}
