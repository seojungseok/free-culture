...
(async()=>{let browser;try{
 ...
 await page.locator('.prep-card').waitFor();assert.equal(await page.locator('.prep-card').count(),1);
 assert.equal(await page.getByRole('link',{name:'캠핑 먹거리',exact:true}).count(),0);
