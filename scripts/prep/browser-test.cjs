...
const originalStore=fs.readFileSync('data/weekend-prep.json');
const port=3257,base=`http://127.0.0.1:${port}`,token=crypto.randomBytes(32).toString('hex');
const child=spawn(process.execPath,[require.resolve('next/dist/bin/next'),'dev','--hostname','127.0.0.1','--port',String(port)],{env:{...process.env,WEEKEND_PREP_ADMIN_TOKEN:token,WEEKEND_PREP_LOCAL_REVIEW:'1'},stdio:['ignore','pipe','pipe'],windowsHide:true});
...
(async()=>{let browser;try{
 ...
 assert.equal(await page.getByRole('navigation',{name:'페이지',exact:true}).count(),1);
 const secondPage=await browser.newPage({viewport:{width,height:1000}});await secondPage.goto(base+'/weekend-prep?page=2',{waitUntil:'domcontentloaded'});await secondPage.locator('.prep-card').first().waitFor();assert.equal(await secondPage.locator('.prep-card').count(),2);await secondPage.close();
 ...
 const tag=page.locator('.prep-tag').first();await tag.focus();assert(await tag.evaluate(e=>e===document.activeElement));assert((await tag.getAttribute('href')).startsWith('https://link.coupang.com/'));assert.equal(await tag.getAttribute('rel'),'sponsored noopener');assert.equal(await tag.getAttribute('target'),'_blank');
 ...
 const cachedProduct=JSON.parse(fs.readFileSync('data/coupangEssentials.json','utf8')).pool[0];const candidate=await (await fetch(base+'/api/weekend-prep/admin',{method:'POST',headers,body:JSON.stringify({action:'lookup',name:cachedProduct.name,affiliateUrl:cachedProduct.url})})).json();assert(candidate.candidate);assert.equal(candidate.verified,false);
 const invalid=structuredClone(source);invalid.products[0].verified=false;
 ...
 await admin.locator('select').filter({has:admin.locator('option', {hasText:'상품을 선택하고 사진을 클릭하세요'})}).first().selectOption(source.products[0].id);const before=await admin.locator('.prep-tag').count();await admin.locator('.prep-image').first().click({position:{x:8,y:8}});assert.equal(await admin.locator('.prep-tag').count(),before+1);
 await admin.getByRole('button',{name:'미리보기 열기'}).click();await admin.locator('.prep-article').waitFor();
 ...
 }finally{if(browser)await browser.close();child.kill();fs.writeFileSync('data/weekend-prep.json',originalStore);}})().catch(e=>{console.error(e);console.error(logs);process.exitCode=1;});
