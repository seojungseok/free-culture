...
import {stopFacts} from './plannerFacts';
...
export function placeStops(): TripStop[] {
  ...
  return getAllPlaces().filter(p=>!/(?:20\d{2}|축제|페어|박람회|비엔날레|페스티벌|전시회)/.test(p.title)).map(p=>({id:"place:"+p.id,title:p.title,href:"/places/spot/"+p.id,area:p.area,
    ...
    .filter(p=>validCoordinates(p.x!,p.y!)).map(stopFacts);
}
function eventStops(): TripStop[] {
  return getAllEvents().filter(e=>e.endDate>=todayYmd() && e.startDate<=addDaysYmd(todayYmd(),60))
    ...
    .filter(e=>validCoordinates(e.x!,e.y!)).map(stopFacts);
}
function foodStops(): TripStop[] {
  return getAllRestaurants().map(r=>({id:"food:"+r.id,title:r.title,href:"/food/spot/"+r.id,area:r.area,
    kind:"food" as const,image:r.image,address:r.addr,x:Number(r.mapx),y:Number(r.mapy)})).filter(r=>validCoordinates(r.x!,r.y!)).map(stopFacts);
}
...
export function plannerOptions(): TripOption[] {
  ...
  for (const area of areas) {
    ...
    const candidates = [...events.filter(e=>e.area===area).slice(0,12),
      ...
      ...pool.filter(p=>p.free).slice(0,6),...pool.filter(p=>p.walk).slice(0,12),...pool.filter(p=>p.culture).slice(0,8),
      ...pool.filter(p=>p.parkingAvailable).slice(0,12),...pool.filter(p=>p.accessText),...food.filter(p=>p.area===area).slice(0,8),...pool.slice(0,8)];
    const seen = new Set<string>();
    for (const s of candidates) if (!seen.has(s.href)) { seen.add(s.href); anchors.push(s); }
  }
  return anchors.map(anchor=>({anchor,nearby:[...nearbyStops(anchor,places,food),...events.filter(e=>e.href!==anchor.href&&e.area===anchor.area&&straightKm(anchor,e)<=3).slice(0,3)]}));
}
