...
## 2026-09-11 최종 변경: 일반30 + 입장권20

기존 사이트 전체20 규칙을 대체한다. 관광지·가을나들이10 + 여행코스10 + 시티투어10, 합계30개와 입장권·체험20개는 독립 한도이며 하루 총50개가 최대다. 검수 실패로 수량을 채우지 않는다. 시티투어는 실제 경유지와 지역이 맞는 내부 관광사진을 글마다 다르게 배정해 대표·목록·OG에 재사용한다. 신규 시티투어도 유효한 고유 사진이 없으면 보류한다.

입장권 공통 제목: [장소명] 이용권 알아보기. 설명: 방문 날짜에 맞는 이용권 가격과 할인 혜택이 있는지 확인해 보세요. 기본 버튼: 이용권 가격 확인 ↗. 복수 상품은 종류를 구분한다. 현재 검증된 기간·조건·출처가 있는 할인만 가격·할인 혜택 확인 ↗으로 표시하고 종료되거나 검증이 오래되면 기본 문구로 되돌린다. 버튼 아래: 와그 예약 페이지로 이동합니다. 기존 주소·예약·제휴 /r/ 링크와 수수료 고지는 유지한다. 이 기준은 공통 TicketLinks 컴포넌트로 공개 글·예약 글·초안에 모두 적용한다.

신규 서버 작성 워크플로의 네트워크 권한 승인 대기 상태는 별개다. 수량 변경만으로 중지된 워크플로를 재활성화하지 않는다.
