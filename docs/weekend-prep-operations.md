...
최신 사용자의 '1분에 10개 이상 금지'를 보수적으로 반영해 **코드상 상한은 직전60초 최대9회·9개**다. 열 번째 조회부터 대기하며 기존 중앙 cap=10도 Lua에서9로 제한한다. 요청 간격 최소6.1초, 실패·재시도 합산, 중앙 상태 불명확 시 차단은 유지한다. 실제 상품 API 호출 없이 모의 검증한다. 아래 초기 구현 설명의 10개 상한은 이전 기준이며 현재 실행에는 이 더 엄격한 상한이 우선한다.
